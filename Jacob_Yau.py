...
def MoveSpindle():
    if FrontSpindle.velocity() == 0:
        ...
        FrontSpindle.stop()
def test_move():
    right_motor1.spin_for(FORWARD,1,TURNS,wait=False)
    right_motor2.spin_for(FORWARD,6,TURNS,wait=False)
def foward(length):
    length = length / 13.5
    ...
    left_motor2.spin_for(FORWARD,length*6,TURNS,wait=False)
def backward(length):
    length = length / 13.5
    ...
    left_motor2.spin_for(REVERSE,length*6,TURNS,wait=False)
def right(degree):
    ...
    right_motor2.spin_for(REVERSE,degree*6,TURNS,wait=False)
def left(degree):
    degree = degree / 90 * 0.96
    ...
    left_motor2.spin_for(REVERSE,degree*6,TURNS,wait=False)
def autonomous():
...
def user_control():
    ...
    #MOTOR CONTROL
        right_motor1.set_velocity((controller_1.axis3.position() - controller_1.axis4.position())/6, PERCENT)
        left_motor1.set_velocity((controller_1.axis3.position() + controller_1.axis4.position())/6, PERCENT)
